...
-- using the hand-crafted forward output to check the module forward output, given the same input
...
-- using the numeric gradients to check the module's back propagated gradients
function gradCheckTestFactory(N, T, D, H, dtype)
  ...
  return function()
    ...
    local dh = torch.randn(#h) -- the gradients at output

    -- acquiring the gradients by module's back propagation
    rnn:zeroGradParameters()
    local dh0, dx = unpack(rnn:backward({h0, x}, dh))
    local dw = rnn.gradWeight:clone()
    local db = rnn.gradBias:clone()

    -- funtions necessary for numeric gradient calculation
    ...
    -- acquiring the gradients by numerical differentiation
...
function tests.noInitialStateTest()
  local N, T, D, H = 4, 5, 6, 7
  local rnn = nn.VanillaRNN(D, H) -- rnn.remember_states is initialized to false
