...
function layer:__init(input_dim, hidden_dim)
  ...
  self.weight = torch.Tensor(D + H, 4 * H) -- input gate, forgate gate, output gate, new content
...
function layer:reset(std)
  ...
  self.bias:zero()
  self.bias[{{self.hidden_dim + 1, 2 * self.hidden_dim}}]:fill(1) -- set the bias of forget gate to 1
...
function layer:updateOutput(input)
  ...
  if not c0 then -- c0 is not provided
    c0 = self.c0
    if c0:nElement() == 0 or not self.remember_states then -- first run or don't remember
      c0:resize(N, H):zero()
    elseif self.remember_states then -- if remember, use the previous evaluated h as c0
  ...
  local bias_expand = self.bias:view(1, 4 * H):expand(N, 4 * H) -- copy the bias for a batch
  ...
  for t = 1, T do
    local cur_x = x[{{}, t}] -- N * D
    local next_h = h[{{}, t}] -- N * H
    local next_c = c[{{}, t}] -- N * H
    local cur_gates = self.gates[{{}, t}] -- N * 4H
    cur_gates:addmm(bias_expand, cur_x, Wx) -- Wx * x + b
    cur_gates:addmm(prev_h, Wh) -- Wh * h + Wx * x + b
    cur_gates[{{}, {1, 3 * H}}]:sigmoid() -- for gates
    cur_gates[{{}, {3 * H + 1, 4 * H}}]:tanh() -- for new content
    local i = cur_gates[{{}, {1, H}}]
    local f = cur_gates[{{}, {H + 1, 2 * H}}]
    local o = cur_gates[{{}, {2 * H + 1, 3 * H}}]
    local g = cur_gates[{{}, {3 * H + 1, 4 * H}}]
    next_h:cmul(i, g) -- gated new contents
    next_c:cmul(f, prev_c):add(next_h) -- new memories
    next_h:tanh(next_c):cmul(o) -- new hidden states
    prev_h, prev_c = next_h, next_c -- save
  end
...
function layer:backward(input, gradOutput, scale)
  ...
  for t = T, 1, -1 do
    ...
    grad_x[{{}, t}]:mm(grad_a, Wx:t()) -- (N by 4H) * (4H by D) = N by D
    ...
    grad_next_h:mm(grad_a, Wh:t()) -- (N by 4H) * (4H by H) = N by H
    grad_next_c:cmul(f)
  end
